...
export default function Pipeline() {
  ...
  const handleAddTask = async (e) => {
    ...
    try {
      ...
      Swal.fire({ icon: "success", title: "Task Created!", timer: 1000, showConfirmButton: false, background: "#131c35", color: "#fff" });
      fetchInvestors();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({ title: "Deleted!", text: "Record removed.", icon: "success", background: "#131c35", color: "#fff" });
        fetchInvestors();
      } catch (err) { Swal.fire({ title: "Error!", text: "Could not delete.", icon: "error", background: "#131c35", color: "#fff" }); }
    }
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({ title: "Success", text: "Updated successfully!", icon: "success", background: "#131c35", color: "#fff" });
      setShowEditModal(false);
      fetchInvestors();
    } catch (err) {
      console.error("Update Error Log:", err.response?.data);
      Swal.fire({ title: "Update Failed", text: err.response?.data?.error || "Check data constraints.", icon: "error", background: "#131c35", color: "#fff" });
    }
  };
  ...
  return (
    <div className="p-4 relative z-10 font-sans text-slate-300 min-h-screen selection:bg-indigo-500/30">

      {/* Header Section */}
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8 bg-[#111827]/40 p-5 rounded-2xl border border-slate-800/80 backdrop-blur-xl shadow-xl">
        <div className="space-y-1">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-gradient-to-tr from-indigo-500/20 to-blue-500/20 border border-indigo-500/30 text-indigo-400">
              <HiBriefcase size={20} />
            </div>
          ...
          <p className="text-xs text-slate-400 pl-11">Manage and track custom investor deal flow stages</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            <select
              ...
              className="appearance-none pl-4 pr-10 py-2.5 bg-[#0f172a]/90 border border-slate-700/80 hover:border-slate-600 rounded-xl text-xs font-semibold text-slate-200 outline-none focus:ring-2 focus:ring-indigo-500/40 transition-all cursor-pointer shadow-inner"
            >
              {pipelines.map((p) => (
          ...
          <button
            ...
            className="p-2.5 bg-[#131c35]/90 border border-slate-700/80 hover:border-indigo-500/50 hover:bg-indigo-500/10 text-indigo-400 rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center"
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-lg shadow-indigo-600/20 active:scale-95 border border-indigo-400/20"
          >
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className="flex gap-5 overflow-x-auto pb-6 scrollbar-thin scrollbar-thumb-slate-800/80 scrollbar-track-transparent">
          {dynamicStages.map((stage) => {
            ...
            return (
              <div 
                key={stage} 
                className="min-w-[320px] max-w-[320px] bg-[#111827]/60 border border-slate-800/80 rounded-2xl flex flex-col h-fit shadow-2xl backdrop-blur-2xl transition-all"
              >
                {/* Stage Header */}
                <div className="p-4 border-b border-slate-800/80 flex items-center justify-between bg-[#131c35]/50 rounded-t-2xl">
                  <div className="flex items-center gap-2.5 truncate">
                    <h3 className="font-bold text-xs text-slate-200 tracking-wider uppercase truncate">{stage}</h3>
                  </div>
                  <span className={`text-[11px] font-extrabold px-2.5 py-0.5 border rounded-lg uppercase tracking-wider shadow-sm ${getStageColor(stage)}`}>
                        ...
                        <Draggable key={inv.id} draggableId={inv.id} index={index}>
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={`bg-[#0f172a]/95 p-4 rounded-xl border transition-all group relative ${
                                snapshot.isDragging 
                                  ? "shadow-2xl shadow-indigo-500/20 border-indigo-500 scale-[1.02] bg-[#131c35]" 
                                  : "border-slate-800 hover:border-slate-700/80 hover:bg-[#131c35]/80 shadow-md"
                              }`}
                            >
                              <div className="flex justify-between items-start">
                                <div className="space-y-1.5 max-w-[85%]">
                                  ...
                                  <div className="pt-1">
                                    <span className="inline-flex items-center gap-1.5 text-[10px] text-indigo-300 font-semibold bg-indigo-500/10 border border-indigo-500/20 px-2.5 py-1 rounded-lg">
                                ...
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity absolute right-3 top-3 bg-[#0f172a] border border-slate-700/80 p-1 rounded-xl shadow-xl backdrop-blur-md">
                                  ...
                                  }} className="p-1.5 text-slate-400 hover:text-amber-400 hover:bg-slate-800/80 rounded-lg transition-all" title="Edit Lead"><HiPencilAlt size={14} /></button>
                                  ...
                                  }} className="p-1.5 text-slate-400 hover:text-emerald-400 hover:bg-slate-800/80 rounded-lg transition-all" title="Link Task"><HiPlus size={14} /></button>

                                  <button onClick={() => handleDelete(inv.id)} className="p-1.5 text-slate-400 hover:text-rose-400 hover:bg-slate-800/80 rounded-lg transition-all" title="Delete"><HiTrash size={14} /></button>
                                </div>
                              </div>
      ...
      {showBoardModal && (
        <div className="fixed inset-0 bg-[#030712]/70 backdrop-blur-md flex justify-center items-center z-[120] p-4">
          <div className="bg-[#111827] border border-slate-800 p-6 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-5 border-b border-slate-800/80 pb-3">
              <div>
                ...
                <p className="text-slate-400 text-[11px] mt-0.5">Configure comma-separated dynamic tracking fields.</p>
              </div>
              <button onClick={() => setShowBoardModal(false)} className="text-slate-400 hover:text-white p-1.5 hover:bg-slate-800/80 rounded-lg transition-all"><HiX size={18} /></button>
            </div>

            <form onSubmit={handleCreateBoard} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Pipeline Name</label>
                <input required placeholder="e.g., Venture Capitalist Pipeline" value={newBoardName} onChange={(e) => setNewBoardName(e.target.value)} className="w-full p-3 bg-[#0f172a] border border-slate-700/80 rounded-xl text-white text-xs focus:ring-2 focus:ring-indigo-500/40 outline-none transition-all" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Columns / Stages (Comma-Separated)</label>
                ...
                <p className="text-[10px] text-slate-500 italic mt-1">Separate columns with commas.</p>
              </div>
              <button 
                type="submit" 
                disabled={isSubmitting}
                className={`w-full py-3 text-white font-bold rounded-xl shadow-lg shadow-indigo-600/20 transition-all uppercase text-xs tracking-wider mt-4 ${isSubmitting ? 'bg-slate-700 cursor-not-allowed text-slate-400' : 'bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500'}`}
              >
      ...
      {/* Add Lead Modal */}
      {showAddModal && (
        <div className="fixed inset-0 bg-[#030712]/70 backdrop-blur-md flex justify-center items-center z-[110] p-4">
          <div className="bg-[#111827] border border-slate-800 p-6 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-5 border-b border-slate-800/80 pb-3">
              <h2 className="text-base font-bold text-white">New Lead Entry</h2>
              <button onClick={() => setShowAddModal(false)} className="text-slate-400 hover:text-white p-1.5 hover:bg-slate-800/80 rounded-lg transition-all"><HiX size={18} /></button>
            </div>

            <form onSubmit={handleAddNew} className="space-y-4">
              <div className="space-y-1">
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Select Fund *</label>
                <select
        ...
        <div className="fixed inset-0 bg-[#030712]/70 backdrop-blur-md flex justify-center items-center z-[110] p-4">
          <div className="bg-[#111827] border border-slate-800 p-6 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-5 border-b border-slate-800/80 pb-3">
              ...
              <button onClick={() => setShowEditModal(false)} className="text-slate-400 hover:text-white p-1.5 hover:bg-slate-800/80 rounded-lg transition-all"><HiX size={18} /></button>
            </div>

            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Select Fund</label>
                <select 
                  className="w-full p-3 bg-[#0f172a] border border-slate-700/80 rounded-xl text-xs font-semibold text-slate-200 outline-none focus:ring-2 focus:ring-indigo-500/40 transition-all cursor-pointer" 
                  value={formData.fundId} 
                  onChange={(e) => setFormData({ ...formData, fundId: e.target.value })} 
        ...
        <div className="fixed inset-0 bg-[#030712]/70 backdrop-blur-md flex justify-center items-center z-[110] p-4">
          <div className="bg-[#111827] border border-slate-800 p-6 rounded-2xl w-full max-w-sm shadow-2xl">
            <div className="flex justify-between items-center mb-5 border-b border-slate-800/80 pb-3">
              ...
              <button onClick={() => setShowTaskModal(false)} className="text-slate-400 hover:text-white p-1.5 hover:bg-slate-800/80 rounded-lg transition-all"><HiX size={18} /></button>
            </div>
            <form onSubmit={handleAddTask} className="flex flex-col gap-4">
              <div className="space-y-1">
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Task Title</label>
                <input autoFocus className="w-full p-3 bg-[#0f172a] border border-slate-700/80 rounded-xl text-white text-xs focus:ring-2 focus:ring-indigo-500/40 outline-none transition-all" placeholder="What needs to be done?" value={taskTitle} onChange={(e) => setTaskTitle(e.target.value)} required />
              </div>
              <div className="space-y-1">
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Description (Optional)</label>
                <textarea className="w-full p-3 bg-[#0f172a] border border-slate-700/80 rounded-xl text-white text-xs focus:ring-2 focus:ring-indigo-500/40 outline-none resize-none transition-all" placeholder="Enter specific task details..." value={taskDescription} onChange={(e) => setTaskDescription(e.target.value)} rows={3} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
              ...
              <button type="submit" className="w-full bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white py-3 rounded-xl font-bold text-xs uppercase tracking-wider shadow-lg shadow-indigo-600/20 transition-all mt-2">Create Task</button>
            </form>
          </div>
        </div>
      )}
